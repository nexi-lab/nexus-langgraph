...
async def get_system_prompt_async(config: RunnableConfig | None = None, role: str = "general", state: dict | None = None) -> str:
    ...
    if config is not None:
        try:
            ...
            return await loop.run_in_executor(
                None, 
                lambda: get_nexus_prompt(config, role=role, state=state, include_opened_file=True)
            )
        except ImportError:
