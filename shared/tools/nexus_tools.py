...
Official Nexus Tools (from nexus_client.langgraph):
...
# Import official Nexus tools from nexus-fs-python (Python 3.11+ compatible)
from nexus_client.langgraph import get_nexus_tools as get_official_nexus_tools
...
def get_nexus_fs_tools():
    ...
    # Exclude query_memories (python, bash last two entries)
    filtered_tools = [
        t for t in official_tools
        if getattr(t, "name", None) not in {"query_memories", "python", "bash"}
    ]
    return filtered_tools

def get_nexus_sandbox_tools():
    """
    Create LangGraph tools that connect to Nexus server with per-request authentication.
    """
    official_tools = get_official_nexus_tools()
    ...
    filtered_tools = [
        t for t in official_tools
        if getattr(t, "name", None) in {"python", "bash"}
    ]
    return filtered_tools
...
def get_web_tools():
    ...
    if os.getenv("TAVILY_API_KEY"):
        web_tools.append(web_search)
    if os.getenv("FIRECRAWL_API_KEY"):
        web_tools.append(web_crawl) 
    return web_tools
